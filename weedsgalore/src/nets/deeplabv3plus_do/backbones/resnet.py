# The code in this file is based on https://github.com/VainF/DeepLabV3Plus-Pytorch and is licensed under the MIT License.
...
class ResNet_DO(nn.Module):
  ...
  # Modified to add dropout to original Resnet50 backbone
  def forward(self, x):
    x = self.conv1(x)
    x = self.bn1(x)
    x = self.relu(x)
...
def _resnet_do(arch, block, layers, pretrained, progress, **kwargs):
  model = ResNet_DO(block, layers, **kwargs)
  if pretrained:
...
def resnet50_do(pretrained=False, progress=True, **kwargs):
  r"""ResNet-50 model from
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
  return _resnet_do('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, progress, **kwargs)
